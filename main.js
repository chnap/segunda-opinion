document.addEventListener('DOMContentLoaded', () => {
    ...
    const DEFAULT_CHECKLIST_TEMPLATE = [
        ...
        { id: 'pathology', name: 'Anatomía patológica completa e inmunohistoquímica' },
    ...
    function createChecklist(statuses = {}, caseData = {}) {
        ...
        const hasQuestion = Boolean(caseData.patientQuestion || caseData.clinical_question);
        ...
        return DEFAULT_CHECKLIST_TEMPLATE.map(item => {
            ...
            if (!status) {
                ...
                else if (item.id === 'patient_question' && hasQuestion) status = 'Aportado';
    ...
    const DEFAULT_CASES = [
        {
            ...
            patientQuestion: '¿Es aconsejable tratamiento de hormonoterapia adyuvante?',
            status: 'NEW_REQUEST',
            updatedAt: '19/08/2026, 07:51',
            dricloudUrl: 'https://pagoseguro.dricloud.net/?URL=dricloud_onc8021',
            checklist: createChecklist({}, { diagnosis: 'Carcinoma ductal', clinical_question: '¿Es aconsejable...?' })
        },
        {
            id: 'ONC-8020',
            priority: 'NORMAL',
            ...
            checklist: createChecklist({ pathology: 'Aportado', imaging: 'Aportado', treatments: 'Aportado', labs: 'Aportado' }, { diagnosis: 'Adenocarcinoma', clinical_question: 'Valoración...' })
        }
    ];
        ...
        try {
            ...
            if (data.ok && Array.isArray(data.cases) && data.cases.length > 0) {
                ...
                cases = data.cases.map(c => {
                    ...
                    return {
                        ...c,
                        checklist: createChecklist(statusMap, { diagnosis: c.pathology || c.diagnosis, clinical_question: c.patientQuestion || c.clinical_question })
                    };
                });
            } else {
                cases = DEFAULT_CASES;
            ...
            console.error('Error al conectar con la base de datos, usando datos predeterminados:', error);
            cases = DEFAULT_CASES;
        }
        renderCases();
    }
    ...
    function showToast(message) {
        ...
        if (!toast) {
            ...
            toast.className = 'fixed bottom-5 right-5 bg-teal-900/90 text-teal-100 border border-teal-500/50 px-4 py-3 rounded-xl shadow-2xl text-xs font-semibold backdrop-blur-md transition-all duration-300 transform translate-y-10 opacity-0 z-50 flex items-center gap-2';
    ...
    function getMissingSummary(c) {
        if (!c || !c.checklist || !Array.isArray(c.checklist)) return '<span class="text-slate-400">Sin datos</span>';
        const missing = c.checklist.filter(item => item && item.status === 'Pendiente');
        if (missing.length === 0) {
    ...
    function getStatusBadgeHTML(status) {
        switch(status) {
            case 'REJECTED': return `<span class="text-xs font-semibold text-rose-400">Denegado</span>`;
            case 'NEW_REQUEST': return `<span class="text-xs font-semibold text-amber-400">Nueva Solicitud</span>`;
            case 'ACCEPTED': return `<span class="text-xs font-semibold text-emerald-400">Aceptado</span>`;
            default: return `<span class="text-xs font-semibold text-slate-300">En curso</span>`;
        }
    }
    ...
    function renderCases() {
        ...
        let filtered = cases.filter(c => {
            ...
            if (currentFilter === 'URGENT') matchesFilter = (c.priority === 'ALTA' || c.priority === 'Urgente');
            else if (currentFilter === 'PENDING_TRIAGE') matchesFilter = (c.status === 'NEW_REQUEST');
            else if (currentFilter === 'DOCUMENTATION_PENDING') matchesFilter = c.checklist && c.checklist.some(i => i.status === 'Pendiente') && c.status !== 'REJECTED';
            else if (currentFilter === 'READY_FOR_CONSULTATION') matchesFilter = c.status === 'ACCEPTED';
            else if (currentFilter === 'REJECTED') matchesFilter = c.status === 'REJECTED';

            const patientName = c.patient || c.patientName || '';
            const pathology = c.pathology || c.diagnosis || '';
            const query = searchQuery.toLowerCase();
            return matchesFilter && (patientName.toLowerCase().includes(query) || pathology.toLowerCase().includes(query));
        });
        ...
        casesBody.innerHTML = filtered.map(c => {
            ...
            if (!Array.isArray(c.checklist)) {
                c.checklist = createChecklist({}, { diagnosis: pathology, clinical_question: c.patientQuestion });
            }
    ...
    window.openCaseModal = function(id) {
        ...
        if (!Array.isArray(c.checklist)) {
            c.checklist = createChecklist({}, { diagnosis: c.pathology || c.diagnosis, clinical_question: c.patientQuestion || c.clinical_question });
        }
                ...
                <div class="lg:col-span-7 space-y-4">
                    ...
                    <div class="bg-slate-900/60 p-4 rounded-xl border border-slate-800/80 space-y-3">
                        ...
                        <div class="space-y-2 max-h-[280px] overflow-y-auto pr-1">
                            ${c.checklist.map(doc => {
                                const isExempt = (doc.id === 'patient_question' || doc.id === 'pathology');
                                return `
    ...
    window.acceptCaseHandler = function(caseId) { updateCaseDecision(caseId, 'ACCEPTED'); };
    ...
    window.submitDenial = function(caseId) {
        ...
        if (!reason) { alert('Indique el motivo de la denegación.'); return; }
    ...
    window.updateCaseDecision = function(caseId, newStatus, reason = '') {
        ...
        if (c) {
            ...
            showToast(newStatus === 'ACCEPTED' ? 'Caso aceptado correctamente.' : 'Caso denegado correctamente.');
            renderCases();
            openCaseModal(caseId);
        }
    };
    ...
    if (closeModal) closeModal.addEventListener('click', window.closeModalHandler);
    if (modal) modal.addEventListener('click', (e) => { if (e.target === modal) window.closeModalHandler(); });
    
    filterButtons.forEach(btn => {
        btn.addEventListener('click', () => {
            filterButtons.forEach(b => b.classList.remove('active', 'border-accent', 'text-accent'));
